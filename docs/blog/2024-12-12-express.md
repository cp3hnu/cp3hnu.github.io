...
> express 当前版本是 5.x，要求 node 18 及以上
...
模版引擎，取代之前分散的 `ejs`、`pug`、`hbs`、`hogan` 4 个选项，支持 8 个模版引擎库 `jade`、`dust`、`ejs`、`hbs`、`hjs`、`pug`、[`twig`](https://github.com/twigjs/twig.js)、`vash`，默认是 `jade`。但是因为 `express-generator` 最新版本 4.16.1 发布于 6 年前，`jade`、`dust`、`hjs`、`vash` 以及用于 [`handlebars.js`](https://github.com/handlebars-lang/handlebars.js) 的 `hbs` 都已经过时，很久没有更新了。推荐使用 [`pug`](https://github.com/pugjs/pug)、[`ejs`](https://github.com/mde/ejs)、[`handlebars.js`](https://github.com/handlebars-lang/handlebars.js)。
...
在他们没有回答之前，我们可以使用 [`expressjs/morgan`](https://github.com/expressjs/morgan) 中间件
...
或者使用 [`debug-js/debug`](https://github.com/debug-js/debug) 手动输出

```sh
$ npm install debug
```

`debug` 导出的是一个函数，需要指定一个调试命名空间调用这个函数。通过设置 `DEBUG=express:server` 运行应用，可以打印 `debug` 的输出
...
Express 4.x 内部就是使用的这个库实现本节前面所讲的调试功能，同时因为 Express 5.x 将这个库移除了，所以 Express 5.x 目前无法实现调试功能。

## 总结

这篇文章我们详细介绍了 Express 框架，下篇文章 [使用 Express 创建 Web 服务（二）](./2025-01-17-express-web.md) 我们使用 Express 创建 Web 服务。
