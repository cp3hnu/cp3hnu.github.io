...
[`tcp_nodelay on;`](https://docs.nginx.com/nginx/admin-guide/web-server/serving-static-content/#enable-tcp_nodelay) 提高网络数据包的实时性，默认是开启的。
...
[同源](https://developer.mozilla.org/en-US/docs/Web/Security/Defenses/Same-origin_policy) 是指协议（protocol）、主机名称（hostname）或 IP 地址、端口（port）都相同。

比如我在 `http://localhost:3000` 的 Web 应用中去请求 `http://localhost:3100` 的后台 API 就会产生跨域问题。

在开发的时候一般使用构建工具的代理来解决跨域问题，比如 [`webpack-dev-server`](https://webpack.js.org/configuration/dev-server/#devserverproxy)、Vite 的 [`server.proxy`](https://cn.vite.dev/config/server-options#server-proxy) 等。
...
同时修改前端代码，将 `fetch("http://localhost:3100/users")` 改为 `fetch("http://localhost:3000/api/users")`。Nginx 将自动为我们做转发，将 `http://localhost:3000/api/users` 请求转发到 `http://localhost:3100/users`。
...
注意需要提高这个 `location` 指令的优先级，防止匹配别的 `location` 指令，导致 js、css 文件无法加载。比如下面的正则表达式 `location` 指令。
...
解决办法是添加一个中间层 `http://localhost:4001/`
