...
- [Docker Hardened Images](https://hub.docker.com/hardened-images/catalog)：最小化、安全、可用于生产环境的镜像，旨在减少攻击面并简化合规性。
...
[Docker Hub](https://hub.docker.com/) 里的镜像有各种各样的 tag，比如 [nginx 镜像](https://hub.docker.com/_/nginx) tag 如下：
...
生成环境建议使用精确版本，比如 `nginx:1.29.6`、`nginx:1.29.6-trixie`。

开发时可以使用 alpine 版本，加快构建速度，比如 `nginx:1.29.6-alpine` 、`nginx:alpine`。
...
> 安装 [Docker Desktop](https://docs.docker.com/get-started/get-docker/) 才有 `docker init` 命令，也可以通过 `brew install docker-init` 安装
...
2. 在 [Docker Hub](https://hub.docker.com/) 里创建个人 Repository 
...
然后容器之间可以通过容器名称进行通信，比如 express 容器使用 `db` 访问 postgres 数据库
...
[Docker 文档 ](https://docs.docker.com/get-started/workshop/08_using_compose/)里有个 `compose.yml` 例子：
...
它会自动创建 network、volume、镜像、容器，并且运行容器。
...
`Dockerfile` 负责创建镜像，`compose.yml` 负责编排容器，比如下面是 React 项目经常用到的 `compose.yml`、`Dockerfile`
...
`Dockerfile` 采用多阶段构建，最终产生 dist + nginx 镜像
...
`compose.yml` 中的 [build 参数](https://docs.docker.com/reference/compose-file/build/) 告诉 Compose 如何从源代码构建应用程序，`context` 定义包含 Dockerfile 的目录路径
...
-  [`kristiyan-velkov/docker-reactjs-sample`](https://github.com/kristiyan-velkov/docker-reactjs-sample)
