...
**方案一：** 对于纵向滚动来说，不要设计 `transformY` 的动画，可以改为 `transformX` 或者 `scale` 这种纵向位置不变的动画。
...
**方案四：** 根据滚动方向设置“离开”时的 Y 偏移。这是最推荐的解决方案。
...
向下滚动导致组件在屏幕上方“离开”时，`transformY(-100px)`，向上滚动导致组件在屏幕下方“离开”时，`transformY(100px)`。

初始时需要 `transformY(100px)`，所以 `direction` 的默认值是 `'up'`。
...
实现效果如下，方块 1 使用了 Scroll-triggered，方块 2 作为参照物。Scroll-triggered 动画如此丝滑。
...
3. 根据 `springScrollY`，使用 `useTransform` 计算 height、left、right、top、radius、padding 等值
