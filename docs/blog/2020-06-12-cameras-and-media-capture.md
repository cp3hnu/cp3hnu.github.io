...
Cameras and Media Capture（下面简写 Capture） 是 AVFoundation 库的一个子系统，提供 photo、video、audio 捕获（capture）功能，可以用于自定义相机、扫码、更高层次的图片/视频/语音处理等。

Capture 架构主要由 session、input 和 output 组成，如下图

![](./assets/capture-architecture.png)

*AVCaptureSession* 将 input 设备捕获的数据，传递给 output 进行处理。*AVCaptureSession* 作为 input 和 output 之间的桥梁。

## 建立 Capture Session

不管 capture 要实现什么样的功能，建立 capture session 的过程是一样的：首先创建一个 *AVCaptureSession* 实例，然后添加 input 和 output，最后运行 session。
...
使用不同的 *AVCaptureOutput* 子类，可以实现不同的功能。*AVCaptureOutput* 有以下这些子类
...
一个 ***CALayer*** 的子类，展示 *AVCaptureSession* 捕获的视频。可以将 *AVCaptureVideoPreviewLayer* 加入 `sublayers`，也可以通过一个 *UIView* 封装 *AVCaptureVideoPreviewLayer*.
...
class ViewControlller: UIViewController {
  ...
  private func configureSession() {
    guard let device = selectCaptureDevice() else { return }
