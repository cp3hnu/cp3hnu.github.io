...
author: cp3hnu
location: ChangSha
summary: 在实际的应用开发中，我们经常会用到 Date 类型，尤其是 Date 类型与 String 类型互相转换更是高频出现。但是因为日期的表示形式多种多样，所以一不小心可能就会出错，尤其是浏览器兼容性问题。这篇文章将详细讲解 Date 类型，尤其是 Date 类型与 String 类型之间的转换。
---

# Date 详解

在实际的应用开发中，我们经常会用到 Date 类型，尤其是 Date 类型与 String 类型互相转换更是高频出现。但是因为日期的表示形式多种多样，所以一不小心可能就会出错，尤其是浏览器兼容性问题。这篇文章将详细讲解 Date 类型，尤其是 Date 类型与 String 类型之间的转换。

## UTC

**UTC** 即[协调世界时](https://zh.wikipedia.org/wiki/%E5%8D%8F%E8%B0%83%E4%B8%96%E7%95%8C%E6%97%B6)（Coordinated Universal Time），是基于原子钟的国际标准时间。UTC 使得不同的地区和系统之间能够统一地解读时间。

UTC 表示的是本初子午线（即 0 度经线）的时间， 而世界各地的时间以本初子午线的时间为基准，再加上各自所在的时区。比如，北京处在东八区，比本初子午线的时间快 8 个小时，也就是 UTC + 8。

**Date** 对象封装了一个整数，这个整数表示从 UTC 1970-01-01 00:00:00 开始的毫秒数。
...
GMT 和 UTC 时间很接近，可以通过下面的两个链接看看 UTC 和 GMT 之间的差别
...
日期的表示方式可以多种多样，而且每个国家的表示方式也各不相同，比如表示2023年5月15日，有的国家是这样的：May 15, 2023，而有的国家是这样的：2023/05/15，所以需要一个通用的日期格式。
...
| DD   | 日，2位数字，01~31                                           | 01       |
| T    | 日期和时间的分隔符                                           | 01       |
| HH   | 时，2位数字，00~23，24:00:00表示第二天的凌晨                 | 00       |
| mm   | 分，2位数字，00~59                                           | 00       |
| ss   | 秒，2位数字，00~59                                           | 00       |
...
在实际的应用中，最常用的就是 String 和 Date 类型互转，比如从后台获取个人出生日期，然后判断这个人现在是多少岁了，我们就需要把 String 转换成 Date。Date 对象提供两个方法将 String 转换成 Date。一个是 Date 构造函数，一个是 `Date.parse` 静态方法。他们接受字符串的格式是一致的。
...
> Only the [date time string format](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date#date_time_string_format) is explicitly specified to be supported. Other formats are implementation-defined and may not work across all browsers.
...
- 如果只有年月日的 date 字符串，则被解析成 UTC 时间，例如，"2023-05-15" 被解析成 "2023-05-15T00:00:00.000Z"。这种情况要小心，因为 `Date.getDay` 这类 API 返回的是本地时间，可能和你想要的结果不一致。所以如果你想要的是本地时间，要在后面添加 "T00:00:00" 再转换成 Date

```js
const date1 = new Date('2023-05-15')          // 2023-05-15T00:00:00.000Z
const date2 = new Date('2023-05-15T10:41:53') // 2023-05-15T02:41:53.000Z
```

ISO 8601 格式中具体哪些字符串能成功转换成 Date，请参考 [Demo](https://gitee.com/cp3hnu/web-demo/tree/master/date-demo)

我还在测试（Safari 16.3 / Chrome 114 / Node 16.14.0 ）中发现：
...
下面是 ISO 8601 非标准字符串在不同环境中的差异

|                                    | Chrome/Node                            | Safari |
| ---------------------------------- | -------------------------------------- | ------ |
| 年超过 4 位，例如 20231            | 有效，但是输出本地时间 20231-01-01午夜 | 有效   |
| 年不足 4 位，例如 202              | 有效，但是输出本地时间 0202-01-01 午夜 | 有效   |
...
- 只有年月日的字符串，被解析为本地时间的午夜，这个与 ISO 8601 Format 有所区别
- 第 3 种格式中的 `day_of_week` 无关紧要，随便什么字母组成的字符串都可以
- 第 3 种格式中的 `month_name` 前 3 位拼写正确就行，且大小写无关
...
在实际的应用中，也常常需要将 Date 类型转 String 类型。Date 转 String 有很多 API 可以使用，但是一般要求转成本地时间 `YYYY-MM-DD HH:mm:ss.SSS` 格式。

我们先来看怎么转成这种格式的 String，然后再看看其它的转 String 的 API。

### YYYY-MM-DD HH:mm:ss.SSS

我们知道 Date 提供了获取各个部分值 API，比如 `getMonth`，因此方法一是获取 Date 各个部分的值，然后组装起来。
...
这是转换成本地时间的字符串，如果你想转换成 UTC 时间的字符串，只需要将 `getXXX` 替换成 `getUTCXXX` 即可。同时，对于获取 UTC 时间的字符串，Date 还提供了 2 个方便的 API `toISOString` 和 `toJSON`。
...
像 `moment` 之类的库，提供了灵活格式化 Date 的方式，比如 `moment().format('YYYY-MM-DD')`，我们可以提供一个简化的实现：
...
const formatDate = (format, date) => {
  const o = {
    ...
    "m+": date.getMinutes(), // 分
    "s+": date.getSeconds(), // 秒
    Q: parseInt(date.getMonth() / 3) + 1, // 季度
  };
  let result = format;
  // 处理年
  const match = /(Y+)/.exec(result);
  if (match) {
    result = result.replace(
      match[0],
      String(date.getFullYear()).substring(4 - match[0].length)
    );
  }
  
  // 处理星期
  const weekMatch = /(E)/.exec(result);
  if (weekMatch) {
    result = result.replace(weekMatch[0], dayOfWeekCodes[date.getDay()]);
  }

  // 处理毫秒
  const msMatch = /(S+)/.exec(result);
  if (msMatch) {
    result = result.replace(
      msMatch[0],
      String(date.getMilliseconds())
        .padStart(3, "0")
        .substring(0, msMatch[0].length)
    );
  }
...
const quarter = formatDate("Q", date) // '2'
const dayOfWeek = formatDate("星期E", date) // '星期一'
// short
const fullShort = formatDate("YY-M-D H:m:s", date) // '23-5-15 10:1:5'
```

如果想要更多的格式转换，可以使用 `moment`、`date-fns`、`dayjs` 等第三方库。

### 本地化

`toLocaleString`、`toLocaleDateString`、`toLocaleTimeString` 3 个函数返回本地化日期字符串。在支持 [`Intl.DateTimeFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat) 的实现中，该方法仅是调用了 `Intl.DateTimeFormat` 方法。

> 📢 官方文档上说：大多数时候，`toLocaleString()` 返回的格式是一致的。但是，这可能在未来发生变化，且不能保证所有语言都能得到一致的格式。输出的变化因实现而异，而且这种变化是规范所允许的。你不应该将 `toLocaleString()` 的结果与静态值作比较。
...
整体的：`dateStyle`、`timeStyle`、`calendar`、`numberingSystem`、`timeZone`、`hour12`、`hourCycle`、`localeMatcher`、`formatMatcher`

各个部分的：`year`、`month`、`day`、`hour`、`minute`、`second`、`fractionalSecondDigits`、`weekday`、`dayPeriod`、`era`、`timeZoneName`
...
`toString`、`toDateString`、`toTimeString` 返回返回一个用本地时区解释的字符串
...
`valueOf` 方法等价于 `getTime`，返回时间戳。我们知道一个对象再转 `number` 时，会调用对象的 `valueOf` 方法。所以我们可以通过下面的方法比较两个 date。
...
// 注意比较相等不能使用 ==/!=，这个是比较两个 date 是否指向同一个对象，而应该这样
const date3 = new Date("2023-05-15T10:10:10");
const date4 = new Date("2023-05-15T10:10:10");
date3 - date4 === 0; // ture
date3.getTime() === date4.getTime(); // true
date3.valueOf() === date4.valueOf(); // true

// 注意比较两个 date 时，时区要一致
const date5 = new Date("2023-05-15");              // UTC 时间
const date6 = new Date("2023-05-15T00:00:00.000"); // 本地时间
date6 - date5 === 0; // false
```

## 常用操作

常用的操作有计算多少天后，多少天前，过去了多长时间等
