...
author: cp3hnu
location: ChangSha
summary: 详细讲解怎么在 JavaScript 中使用正则表达式，也会介绍 String 与正则表达式相关的 API，最后推荐与正则表达式相关的一些工具或网站以及一些常用的正则表达式。
---
...
这篇文章将介绍怎么在 JavaScript 中使用正则表达式，也会介绍 String 与正则表达式相关的 API，最后推荐与正则表达式相关的一些工具或网站以及一些常用的正则表达式。
...
`test()` 参数必须是字符串，非字符串参数会强制转换成字符串，比如 `undefined` 将被转换成 `"undefined"`。
...
当带有 global (`g`) 或 sticky (`y`) flag 时，执行 `test()` 会修改 `lastIndex` 属性，再次执行时，会从 `lastIndex` 位置开始进行匹配，并且无论要匹配的字符串是否发生变化。
...
和 `test()` 一样， `exec()` 的参数也必须是字符串，非字符串参数会强制转换成字符串。

`exec()` 比 `test()` 的功能要强大的多。当找到匹配项时，它返回一个字符串数组，其中第一项是匹配的子字符串，后面跟着的是捕获组字符串，除此之外它还有以下 4 个属性。

| 属性      | 说明                                                         |
| --------- | ------------------------------------------------------------ |
| `index`   | 匹配的子字符串在参数字符串的开始位置                         |
| `input`   | 参数字符串                                                   |
| `groups`  | 命名的捕获组对象，捕获组名称为对象的属性名                   |
| `indices` | 匹配的子字符串和捕获组字符串位置信息，必须带有 hasIndices (`d` ) flag 才会返回 |
...
接下来介绍不同的 flag 怎样影响正则表达式的匹配。`i`、`s` 模式很好理解，无需说明。

#### global (`g`) 和 sticky (`y`)

global 和 sticky 模式，都将修改 `RegExp` 对象的 `lastIndex` 属性，使得匹配往下推进，当没有找到匹配项时，都会重置 `lastIndex` 为 0。不同的是 global (`g`) 会搜索 **`lastIndex` 及之后**位置的字符串，而 sticky (`y`) 必须在 **`lastIndex` 位置**进行匹配。
...
| `matchAll`   | 匹配遵循 `y` flag，但是返回一个迭代器，该迭代器生成所有的匹配项 |
| `split`      | 都被忽略                                                     |

总结一下是，除 `search()` 和 `split()` 之外， 其它方法在正则表达式同时有 `g` 和 `y` flag 时，匹配遵循 `y` flag 的原则，但是会往下进行匹配，直到找不到匹配项为止。
...
使用 `d` flag，执行 `exec()` 时返回匹配的子字符串和捕获组字符串位置信息，如 `exec()` 的例子所示。 
...
最重要的是，ES2018 引入 `\p{…}` 和  `\P{…}` unicode 字符类转义（Unicode character class escape）。

它的原理是每个 unicode 字符都有一组描述它的属性。例如，字符 `a` 具有 `General_Category` 的 `Lowercase_Letter` 属性和 `Script` 的 `Latn` 属性。 `\p` 和 `\P` 转义序列允许我们根据字符的属性来匹配字符。
...
再举一个中文正则表达式的例子，以前我们写一个匹配中文的是

```javascript
/[\u4e00-\u9fa5]/
```

但是这个正则表达式已经过时了，因为 unicode 字符一直在更新，Unicode 8.0 添加的 109 号化学元素【鿏】，其码点是 `9FCF`，就不在这个范围内，所以一种现代的表示中文字符的正则表达式是

```javascript
/\p{Unified_Ideograph}/u
```

关于 unicode 字符类转义的更多详情请参考 [proposal-regexp-unicode-property-escapes](https://github.com/tc39/proposal-regexp-unicode-property-escapes)、[Unicode character class escape](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions/Unicode_character_class_escape)。

#### unicodeSet (`v`)

UnicodeSet (`v`) 是 Unicode (`u`) 的增强版

>  unicodeSet 只能在新版浏览器才能使用

- `\p` 转义序列还可以用于匹配字符串的属性，而不仅仅是字符。
- [character class](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions/Character_class) 已升级，允许交集、并集和减法，以及匹配多个 Unicode 字符。
- 字符类补码 `[^...]` 语法构造补码类，而不是否定匹配结果，避免了一些不区分大小写的匹配的混淆行为。有关详细信息，请参考 [Complement classes and case-insensitive matching](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions/Character_class#complement_classes_and_case-insensitive_matching)。
...
搜索与正则表达式匹配的子字符串，如果找到了，返回第一个匹配项的索引，如果没有找到，则返回 -1。

参数必须是 `RegExp`，如果不是，则使用 `new RegExp()` 进行转换。

> global (`g`) flag、`lastIndex` 没有作用
...
> 如果 `pattern`  是字符串，或者正则表达式中不存在相应的捕获组，则替换为字符本身。
>
...
`replace()` 只替换一次，如果想要全部替换，需要使用 global (`g`) flag 的正在表达式。
...
搜索与正则表达式匹配的所有子字符串，并用替换项进行替换，返回替换后的字符串。
...
如果 `pattern` 是 空字符串，则替换将插入到每个 UTF-16 代码单元之间，类似于 `split()` 行为
...
返回与正则表达式匹配的子字符串信息，如果没有找到返回 `null`。

参数和 `search()` 一样必须是 `RegExp`。

返回值应正则表达式是否带有 global (`g`) flag 而异，如果带有 global (`g`) flag，则返回匹配的字符串数组，如果没有 global (`g`) flag，则返回第一个匹配项及其相关的捕获组信息，等同于 `regexp.exec(str)`。

没有 global (`g`) flag 的正则表达式
...
以匹配项分割字符串，参数 `separator` 可以是字符串或者 `RegExp`，参数 `limit` 可选，决定分割返回数组的个数。
...
[RegexLearn](https://regexlearn.com/) 是教你一步一步学习怎样写正则表达式，类似的还有 [RegexOne](https://regexone.com/).
