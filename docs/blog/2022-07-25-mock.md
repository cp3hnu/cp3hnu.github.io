...
- 期待 `findFirst` 有更简便的写法，就像 [Mirage](https://miragejs.com/)
  ...
  ```
  因为大部分情况下都是通过 `primaryKey` 找到所需数据。

## Mirage

[Mirage](https://miragejs.com/) 通过 "stub" 系统的 `fetch` and `XMLHttpRequest` 来实现 mock，这个 "stub" 我理解为：

`Mirage` 重写了系统的 `fetch` and `XMLHttpRequest` 相关的方法，我们程序在使用 `fetch` and `XMLHttpRequest` 做网络请求的时候其实调用的是  `Mirage` 重写的方法，这样  `Mirage` 就能拦截 request，提供 mock response。这样也解释了在使用  `Mirage` 的时候，我们在 `Network` 面板里看不到发出的网络请求。
...
`Models` 定义数据模型和数据之间关系，例如下面定义 `blogPost` 和 `author` 两个模型，其中  `blogPost` 有一个 `author` 属性指向  `author` 模型
...
结合 Models，返回 facker 数据，避免硬编码数据。
...
[JSON Server](https://github.com/typicode/json-server) 与 [Mock Service Worker](https://mswjs.io/docs/) 不同，它开启一个 http server，来响应 mock 请求，只支持 REST。它支持两种方式
...
现在你访问 http://localhost:3000/posts/1, 就能得到
...
1. JSON 数据不方便编辑，一个错误的符号可能会导致 json 数据格式错误，数据量越大越容易产生错误。

2. JSON 数据硬编码，不能使用 faker 等工具自动生成假数据。

3. 数据模型只支持 `id`，如果后台 API 使用的是 `userId` 等，我们就还得修改代码。

4. 返回的 JSON 数据可能和后台 API 返回的数据格式不符，一般后台 API 会多封装一层。
...
这个时候为了兼容 mock 数据请求，只能修改网络层的封装代码了。

5. 自动生成的请求路由可能并不匹配后台 API，比如新增，有的后台开发人员喜欢用 `/users/add`。

`JSON Server` 也支持 js 文件
...
这种方式可以使用 faker，可以随意自定义请求路由，自定义响应，但是

- 如果要保存新增、更新、删除的操作结果，需要自己实现
- 不支持热更新，添加请求路由之后需要重启服务
- 不方便调试
...
2. 可以自定义模型的主键
3. 查询数据的方式很灵活
4. 支持热更新，能 debugger 调试
5. 支持排序、分页
6. 支持 Node / Browser、支持 REST / GraphQL
...
1. 创建了一个简易的后台服务，适合非 web 应用，比如原生的 iOS、Android 应用
2. 支持过滤、分页、排序
3. 支持热更新
...
2. 返回的 json 数据不能封装，可能与真实的后台 API 不一致

3. 自动生成的请求路由可能并不匹配后台 API
