...
上篇文章 [Learn MCP](/2025/09/11/mcp/) 我们介绍了 MCP 协议、连接 MCP 服务以及构建自己的 MCP 服务，这篇文章我将推荐一些常用的 MCP 服务，以及怎么安装和使用这些 MCP 服务。平台包括 Cursor、Claude Desktop、Windsurf、VS Code 等。ChatGPT 目前还不支持 MCP 服务。
...
比如安装 [GitHub MCP Server](https://github.com/github/github-mcp-server) 
...
接下来我们看看怎么在 Cursor 和 Claude 里安装与使用 [`GitHub MCP Server`](https://github.com/github/github-mcp-server)。
...
![](./assets/mcp-server-context7-cursor.png)
...
Cursor 可以通过插件安装 `Figma`。在 [插件市场](https://cursor.com/marketplace) 搜索 `Figma`，点击安装。然后在 `Cursor Setttings` -> `Tools & MCPs`，找到安装的 `figma`，点击旁边的 `Connect`，跳转到 Figma 进行授权。
