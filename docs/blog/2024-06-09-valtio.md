...
Valtio 异步和同步没有区别，定义一个异步 action，然后直接修改 state 即可。
...
Valtio 支持 Promise state 属性，当调用 `snapshot()` 方法时，它会被 resolve 或者 reject。当 React 组件访问时， 组件会被挂起，此时父组件负责回退状态和错误处理。
...
- [`derive-valtio`](https://github.com/valtiojs/derive-valtio)
