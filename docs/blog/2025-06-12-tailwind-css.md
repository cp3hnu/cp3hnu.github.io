...
为了支持断点范围，Tailwind CSS 也提供了相应的 maximum width 的变体 `max-`
...
用法与视口（viewport）断点类似，除了需要使用 `@container` 确定基于哪个父容器。

当要基于多个父容器时，可以使用 `@container/{name}` 进行命名。
...
**但是需要注意的是**，正如 [工作原理](#工作原理) 所介绍的，Tailwind CSS 的功能类是动态生成的，相应的主题变量也是动态生成的，如果项目中没有使用相应的功能类，主题变量也就不存在。
...
**一、Tailwind CSS 中自定义的 `@rules`，比如 `@apply`，VSCode 不能识别 `Unknown at rule @themecss(unknownAtRules)`**
...
2. 建立新的语言模式。`cmd` + `shift` + `p`，输入 `Change Language Mode`，然后在下拉列表中选择 "Tailwind CSS"

**二、Tailwind CSS 的 `--tw-` 变量的用处？**
...
`--tw-` 是 Tailwind CSS 的内部“钩子”变量，用来灵活控制复杂组件的样式行为。

`--tw-ring-color: var(--color-white);` 设置了 Tailwind 的 ring 效果所使用的颜色变量。

当你使用这样的类时

```react
<div class="ring-2 ring-white"></div>
```

Tailwind 实际会渲染成类似这样的 CSS：
...
所以设置 `ring-white` 的本质，是给 `--tw-ring-color` 这个变量赋值，然后 Tailwind 的 `ring-*` 系列会引用它

**为什么 Tailwind 用这种方式？**

1. **可组合性**：你可以自由组合 `ring-`、`ring-offset-` 等类，它们都通过 CSS 变量组合在一起。
2. **优化性能**：生成的类不会重复定义样式，而是利用变量“传值”。
3. **便于主题定制**：可以通过 CSS 变量改变整个组件样式，而不需要每个地方都改类名。

**三、Tailwind CSS 的 `--tw-` 变量与主题变量（比如 `--color-white`）的区别？**

1. 职责分离
	
	`--color-white` 是一个主题变量，表示“白色是什么颜色”，由设计系统定义。
	`--tw-ring-color` 是 Tailwind 的“渲染钩子变量”，表示“当前 ring 用的颜色”。Tailwind 不关心颜色的值是什么，只关心怎么“应用”
	颜色，所以它不会直接用 `--color-ring`。
	
2. 支持原子类组合

   `--tw-` 主要用于原子类组合，比如 `ring-white ring-2`。`ring-white` 设置 `--tw-ring-color` 变量，而 `ring-2` 使用 `--tw-ring-color` 变量，它们一起组成了元素的样式。
