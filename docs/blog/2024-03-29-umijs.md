...
Umijs 搭建的项目不能正确识别 React Hook 缺失，需要安装 [`eslint-plugin-react-hooks`](https://www.npmjs.com/package/eslint-plugin-react-hooks)。顺便还可以安装 [`eslint-plugin-react`](https://www.npmjs.com/package/eslint-plugin-react)。
